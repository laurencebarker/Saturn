...
|---|---|---|---|---|
| Script runner with live output | `index.html` | `POST /run` | `/opt/saturn-go/scripts/*` launched by backend | N/A |
| Script catalog and flag metadata | `index.html` | `GET /get_scripts`, `GET /get_flags`, `GET /get_versions` | Reads `config.json` | `/var/lib/saturn-web/config.json` |
...
| Transactional appliance update | `backup.html` | `GET/POST /update_policy`, `POST /update_start`, `GET /update_status`, `POST /update_rollback` | `git fetch`, `git worktree add/remove`, `curl` health check, snapshot `tar` | `update_policy.json`, `update_state.json`, `snapshots/`, `repo-staging/` |
| Pre-update snapshots + retention | `backup.html` status panel | Part of update workflow | `tar` snapshot + prune logic | `snapshots/` |
