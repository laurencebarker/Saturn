...
// for details of using libgpiod V2 please see:
// https://github.com/brgl/libgpiod/blob/master/examples/get_multiple_line_values.c
// /////////////////////////////////////////////////////////////
...
static const unsigned int PBIOPins[VNUMGPIO] = {20, 26, 6, 5, 4, 21, 7, 9,
...
//
// check if panel is present by looking for the i2c device
//
...
//
// tick function; runs as a thread.
//
...
//
// setup GPIO pins.
// VFO uses 2 pins, one edge triggered and the other level sensed.
// pushbuttons use 20 input lines, all level sensed. 
//
void SetupG2PanelGPIO(void)
{
    ...
    /* VFO lines: gpio17 with rising-edge events, gpio18 as input */
    {
    ...
    /* Bulk inputs for encoders/pushbuttons */
    {
        ...
        // this seems to need to be called once, per input
        for(Cntr=0; Cntr < VNUMGPIO; Cntr++)
        {
            Retval = gpiod_line_config_add_line_settings(lc, &PBIOPins[Cntr], 1, ls_in);
            if(Retval != 0) break;
        }
        if (Retval == 0)
        {
