...
- **Script Execution**: Runs scripts defined in config.json (e.g., update-G2.py version 2.4, update-pihpsdr.py version 1.7, log_cleaner.sh, restore-backup.sh) with configurable flags.
...
- **Modular Setup**: Split into scripts for easier maintenance:
  ...
  - create_files.sh (version 1.6): Creates index.html, saturn_update_manager.py, themes.json, and desktop shortcut; backs up existing config.json if needed.
...
2. **Interface Features**:
   - **Script Versions**: Optionally displays versions of saturn_update_manager.py (2.22), update-G2.py (2.4), update-pihpsdr.py (1.7), and others via a "Show Versions" checkbox (default off).
