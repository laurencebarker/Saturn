...
do
  rm -f data/output_datafile${i}_8K.bin
  echo "Info: DMA setup to read from c2h channel $i. Waiting on write data to channel $i."
  $tool_path/dma_from_device -d /dev/xdma0_c2h_${i} -f data/output_datafile${i}_8K.bin -s $transferSize -c $transferCount &
done
...
do
  ...
  $tool_path/dma_to_device -d /dev/xdma0_h2c_${i} -f data/datafile${i}_8K.bin -s $transferSize -c $transferCount &
done
...
do
  ...
  cmp data/output_datafile${i}_8K.bin data/datafile${i}_8K.bin -n $transferSize
  returnVal=$?
  if [ ! $returnVal == 0 ]; then
