...
(
  ...
  parameter IN_WIDTH   = 18, //ADC bitwidth
...
(
  input wire aresetn,
  input wire aclk,
  input signed [FREQ_WIDTH-1:0] frequency,
...
localparam OUT_WIDTH = WR;                  //24-bit output width
  ...
  for (n=0; n<=(STG-2); n=n+1)
    ...
    wire Z_sign = Z[n][WZ-1-n];
	 ...
	 begin
      ...
      X[n+1] <= !aresetn ? 0 : (Z_sign ? X[n] + Y_shr + Y[n][n] : X[n] - Y_shr - Y[n][n]);
      Y[n+1] <= !aresetn ? 0 : (Z_sign ? Y[n] - X_shr - X[n][n] : Y[n] + X_shr + X[n][n]);

      //update angle
      if (n < STG-2)
        begin : angles
        Z[n+1][WZ-2-n:0] <= !aresetn ? 0 : (Z_sign ? Z[n][WZ-2-n:0] + atan : Z[n][WZ-2-n:0] - atan);
        end
      end
    end
endgenerate
...
generate
  if (OUT_WIDTH == WR)
    begin
    assign m_axis_tdata[23:0] = X[STG-1];
    assign m_axis_tdata[47:24] = Y[STG-1];
    end
    ...
    assign m_axis_tdata[23:0] = rounded_I;
    assign m_axis_tdata[47:24] = rounded_Q;
    end
endgenerate

assign m_axis_tvalid = 1;                       // always output a sample
